...
interface SectionProps {
  ...
  layout?: "default" | "side-by-side"; // Add new layout prop
}

const Section = forwardRef<HTMLElement, SectionProps>(
  (
    { id, title, subtitle, description, children, className, align, layout = "default" },
    forwardedRef
  ) => {
    ...
    return (
      <section id={id} ref={ref}>
        <div className={cn("relative mx-auto max-w-screen-xl px-4", className)}>
          ...
          <div className={cn(
            "mt-8",
            layout === "side-by-side" && "grid grid-cols-1 md:grid-cols-2 gap-8"
          )}>
            {children}
          </div>
        </div>
