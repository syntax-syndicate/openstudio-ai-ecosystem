'use client';
import { ErrorBoundary } from '@/app/(organization)/chat/components/tools/error-boundary';
import type { lineChartSchema } from '@/tools/line-chart';
import {
  ChartContainer,
  ChartLegend,
  ChartTooltip,
...
const trimLabel = (label: string | number, maxLength = 5): string => {
    ...
    const combinedData =
      xData?.map((xValue, index) => {
        ...
        ySeriesData?.forEach((series) => {
          ...
          dataPoint[yKey] = isNaN(yValue) ? 0 : yValue;
          dataPoint[`trimmed_${yKey}`] = trimLabel(isNaN(yValue) ? 0 : yValue);
        });
        return dataPoint;
      }) ?? [];
